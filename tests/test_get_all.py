from tests.conftest import *  # Импорт тестовых данных и токенов из модуля
import uuid

async def test_pagination(client, headers, pagination_params):
    """Тест пагинации (offset и page_size) для списка задач"""
   
    # Создаем 10 тестовых задач
    for i in range(10):  
        unique_id = str(uuid.uuid4())[:8]
        task_name = f"Task_Pagination_{unique_id}"
        task_data = {
            "name": task_name,
            "status": False,
        }

        response = await client.post(
            "/tasks/", 
            json=task_data,  
            headers=headers 
        )
    
    # Тест 1: Первая страница (offset=0, page_size=5) -> 5 задач
    response = await client.get(
        "/tasks/",
        params=pagination_params["first_page"],
        headers=headers
    )
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 5  
    
    # Тест 2: Вторая страница (offset=5, page_size=5) -> следующие 5 задач
    response = await client.get(
        "/tasks/",
        params=pagination_params["second_page"],
        headers=headers
    )
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 5  
    
    # Тест 3: Смещение больше количества задач (offset=2000) -> пустой список
    response = await client.get(
        "/tasks/",
        params=pagination_params["large_offset"],
        headers=headers
    )
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 0  
    
    # Тест 4: Отрицательный offset -> 422 Validation Error
    response = await client.get(
        "/tasks/",
        params=pagination_params["negative_offset"],
        headers=headers
    )
    assert response.status_code == 422  
    
    # Тест 5: Отрицательный page_size -> 422
    response = await client.get(
        "/tasks/",
        params=pagination_params["negative_page_size"],
        headers=headers
    )
    assert response.status_code == 422  
    
    # Тест 6: Нулевой page_size -> 422
    response = await client.get(
        "/tasks/",
        params=pagination_params["zero_page_size"],
        headers=headers
    )
    assert response.status_code == 422 
    
async def test_filter(client, headers, pagination_params):
    """Тест фильтрации задач по статусу (status параметр)"""
   
    # Создаем 10 тестовых задач (все со статусом False)
    for i in range(10):  
        unique_id = str(uuid.uuid4())[:8]
        task_name = f"Task_Filter_{unique_id}"
        task_data = {
            "name": task_name,
            "status": False,
        }

        response = await client.post(
            "/tasks/", 
            json=task_data,  
            headers=headers 
        )

    # Создаем 10 тестовых задач (все со статусом True)
    for i in range(10):  
        unique_id = str(uuid.uuid4())[:8]
        task_name = f"Task_Filter_{unique_id}"
        task_data = {
            "name": task_name,
            "status": True,
        }

        response = await client.post(
            "/tasks/", 
            json=task_data,  
            headers=headers 
        )
     
    # Тест 1: status=1 (только активные задачи, status=True) -> все задачи должны иметь status=True
    response = await client.get(
        "/tasks/",
        params=pagination_params["status_1"],
        headers=headers
    )
    assert response.status_code == 200
    datas = response.json()
    # Проверяем, что все возвращенные задачи имеют статус True
    assert all(data["status"] == True for data in datas)
     
    # Тест 2: status=2 (только завершенные задачи, status=False) -> все задачи должны иметь status=False
    response = await client.get(
        "/tasks/",
        params=pagination_params["status_2"],
        headers=headers
    )
    assert response.status_code == 200
    datas = response.json()
    # Проверяем, что все возвращенные задачи имеют статус False
    assert all(data["status"] == False for data in datas)
    